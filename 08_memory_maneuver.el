    ...
    (while (< (length children) child-nodes)
      (let* ((result (build-node rest))
        ...
        (setq children (cons child-node children)
              rest remaining)))
    (cons (make-node :child-nodes (reverse children)
...
(cl-defun node-value (node)
  (if (null (node-child-nodes node))
      (sum-metadata node)
    (->> (node-metadata-entries node)
         (--reject (equal it 0))
         (-map (lambda (x) (nth (- x 1) (node-child-nodes node))))
         (--reject (null it))
         (-map 'node-value)
         (-reduce '+))))

(node-value (make-node :child-nodes '() :metadata-entries '(1 2 3)))
(node-value (make-node :child-nodes (list (make-node :child-nodes '() :metadata-entries '(3 5 7))) :metadata-entries '(1)))
(node-value (build-tree (read-puzzle (example-input))))

(node-value (build-tree (read-puzzle (puzzle-input))))
