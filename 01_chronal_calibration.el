...
; anomalies with stars. The other bad news is that we only have one device and you're the best person for the job! Good
; lu--" She taps a button on the device and you suddenly feel like you're falling. To save Christmas, you need to get
...
; examples
(= 3 (resulting-frequency '("+1" "+1" "+1")))
(= 0 (resulting-frequency '("+1" "+1" "-2")))
(= -6 (resulting-frequency '("-1" "-2" "-3")))

(defun read-puzzle (filePath)
  "Return a list of lines of a file at filePath."
  (with-temp-buffer
    (insert-file-contents filePath)
    (split-string (buffer-string) "\n" t)))

(resulting-frequency (read-puzzle "01_input"))
...
(defun first-repeated-frequency-change (readings)
  "Returns the first resulting frequency that is duplicated from cycling though the READINGS"
  (let
      ((current-readings (mapcar 'string-to-int readings))
       (current-resulting-frequency 0)
       (resulting-frequencies '()))
    (while (not (memq current-resulting-frequency resulting-frequencies))
      (setq resulting-frequencies (cons current-resulting-frequency resulting-frequencies))
      (setq current-resulting-frequency (+ current-resulting-frequency (car current-readings)))
      (setq current-readings (cdr current-readings))
      (if (null current-readings)
          (setq current-readings (mapcar 'string-to-int readings))))
